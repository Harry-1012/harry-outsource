...
// Site default language is NOW Chinese (zh-CN). We treat Chinese as original text; any other selection triggers Google Translate.
...
(function() {
  ...
  const BASE_LANG = 'zh-CN';    // Original site language (now Chinese)
  // Supported languages list (Google Translate codes). Put Chinese first as base.
  const LANG_LIST = [
    { code: 'zh-CN', label: '中文' },
  ...
  const BUTTON_ICON = '🌏';
  const DEFAULT_BUTTON_TEXT = '语言'; // fallback if label lookup fails
  ...
  function normalizeLang(code) {
    ...
    // Treat zh/zh-CN variants as base
    if(lower === 'zh' || lower === 'zh-cn' || lower.startsWith('zh-cn-')) return BASE_LANG;
    return code;
  }
  ...
  window.googleTranslateElementInit = function(){
    ...
    translatorReady = true;
    drainReadyQueue();
    // If initial stored language is NOT the base Chinese, trigger translation
  ...
  if(current !== BASE_LANG) return; // already not default
    ...
    function pick(){
      for(const raw of langs){
        if(!raw) continue;
        if(raw.startsWith('zh')) return null; // keep base (Chinese)
        // exact
